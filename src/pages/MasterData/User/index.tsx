...
import { getUsers, type GetUsersResponse } from '@/services/auth/auth.service';
...
export default function UserPage() {
  ...
  const { data, isLoading, error } = useQuery<GetUsersResponse>({
    ...
    queryFn: () => getUsers({ role: roleFilter, search: debouncedSearch, page, limit }),
  ...
  return (
    <div className='text-accent-foreground'>
      ...
      <div className='mt-5 grid grid-cols-2 xl:grid-cols-4 gap-5 justify-between items-center'>
        <StatCard icon={User} title='Total Users' value={summary?.totalUsers ?? 0} />
        <StatCard icon={UserStar} title='Online Users' value={summary?.totalAktifUser ?? 0} />
        <div className='flex justify-between bg-sidebar border rounded-sm items-center w-full h-[95px] px-4 hover:bg-gray-100 hover:-translate-y-0.5 transition-all col-span-2'>
          ...
          <div>
            ...
            <p className='text-2xl text-right'>{summary?.totalSuperAdmin ?? 0}</p>
          </div>
          <div>
          ...
          <div>
            ...
            <p className='text-2xl text-right'>{summary?.totalMaintenance ?? 0}</p>
          </div>
        </div>
      </div>
